...
class TokenView(views.TokenView):
    def _get_access_token_jwt(self, user, content):
        extra_data = {}
    ...
    def _is_jwt_config_set():
        ...
        private_key = getattr(settings, "JWT_PRIVATE_KEY_RSA", None)
        id_attribute = getattr(settings, "JWT_ID_ATTRIBUTE", None)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if token:
            content["access_token"] = self._get_access_token_jwt(token.user, content)
        try:
